...
if cv2.ocl.haveOpenCL():
    ...
    console.print(f"OpenCL: ✓", style="green")
else:
    raise RuntimeError("OpenCL not available. Install OpenCV with OpenCL support: brew install opencv")
...
class VideoQualityAnalyzer:
    def __init__(
        ...
        console.print(f"Video Info:", style="bold green")
        ...
        with Progress(
            ...
            for frame_idx in range(self.frame_count):
                ...
                if len(frame_buffer) >= self.window_frames:
                    sorted_frames = sorted(frame_buffer, key=lambda x: x.quality, reverse=True)
...
def extract_frames(
    ...
    with Progress(
        ...
        for frame in best_frames:
            output_path = frames_dir / f"frame_{frame.index:06d}_{frame.quality:.0f}.jpg"
