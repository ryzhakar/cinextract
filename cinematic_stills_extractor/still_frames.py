...
class FrameSelectionConfig:
    ...
    quality_penalty_after_selection: float = 0.3  # How much to raise threshold after selection
...
def select_frames(
    frames: pl.DataFrame,
    config: FrameSelectionConfig = FrameSelectionConfig()
) -> pl.DataFrame:
    """
    ...
    with_metrics = frames.with_columns([
        ...
        pl.col("quality").rolling_max(
            window_size=config.local_window_size,
            center=True,
            min_samples=1
        ).fill_null(pl.col("quality")).alias("local_max_quality"),
        
        # Local mean quality in window, fill null with current quality
        pl.col("quality").rolling_mean(
            window_size=config.local_window_size,
            center=True,
            min_samples=1
        ).fill_null(pl.col("quality")).alias("local_mean_quality"),
    ])
    ...
    for frame in df_dict:
        ...
        gap_adjustment = min(gap * config.quality_boost_per_gap, config.max_gap_adjustment)
        penalty = current_penalty * (config.penalty_decay_rate ** gap)
        threshold = max(
            config.min_quality_threshold,
            median_quality - gap_adjustment + penalty
        )
        
        # Determine if frame should be selected
        is_local_max = (
            quality >= frame["local_max_quality"] * config.local_max_ratio and 
            quality >= frame["local_mean_quality"] * config.local_mean_ratio
        )
        
        should_select = (
            gap >= config.min_gap and
            (
                (is_local_max and quality >= threshold) or
                gap >= config.max_gap
            )
        )
