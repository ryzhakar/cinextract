...
class AestheticPredictor:
    def __init__(self, device: str = "mps") -> None:
        ...
        self.clip_model = (
            open_clip.create_model(  # Use the same CLIP model as your embeddings
                "ViT-L-14", pretrained="openai"
            ).to(self.device)
        )

        weights_path = "ava.pth"  # Or the .pth file you want to use
        self.aesthetic_predictor_weights = torch.load(
            weights_path, map_location=self.device
        )

        self.fc = torch.nn.Linear(768, 1).to(
            self.device
        )  # Input size MUST match your embeddings (ViT-L-14 outputs 768)
    ...
    def score_exemplars(self, embeddings_file: Path) -> Iterator[ExemplarScore]:
        ...
        for row in df.iter_rows(named=True):
            embedding = np.frombuffer(row["embedding"], dtype=np.float32).reshape(
                768
            )  # Reshape to 768
            embedding = (
                torch.from_numpy(embedding).to(self.device).unsqueeze(0)
            )  # Correct way to create tensor
...
if __name__ == "__main__":
    ...
    embeddings_file = Path(
        "output/clip_embeddings.parquet"
    )  # Your existing embeddings file
