...
class FrameDataset(Dataset):
    def __init__(self, frames_dir: Path) -> None:
        self.frame_paths = sorted(
            frames_dir.glob("*.jpg"), key=lambda p: int(p.stem.split("_")[1])
        )
...
class CLIPEncoder:
    ...
    def encode_frames(
        self, frames_dir: Path, batch_size: int = 32
    ) -> Iterator[FrameEmbedding]:
...
if __name__ == "__main__":
    ...
    def main(
        ...
        output_dir: Path = typer.Option(
            "output", help="Output directory for embeddings"
        ),
    ) -> None:
